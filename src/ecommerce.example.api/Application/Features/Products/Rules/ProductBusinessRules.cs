...
public class ProductBusinessRules : BaseBusinessRules
{
    ...
    public Task ProductShouldntExistWhenSelected(Product? product)
    {
        if (product == null)
            throw new BusinessException(ProductsBusinessMessages.ProductNotExists);
        return Task.CompletedTask;
    }
    public async Task ProductCodeShouldntExistWhenSelected(Product? product)
    {
        var selectedProduct = await _productRepository.GetAsync(i=>i.ProductCode == product.ProductCode);
        if (selectedProduct != null)
            throw new BusinessException(ProductsBusinessMessages.ProductAlreadyExists);
    }
